<template>
  <div>
    <button
      class=" btn bg-primary rounded-full absolute confirm-cart-btn"
      @click="ConfirmCart()"
    >
      <svg
...
export default {
  computed: mapState(["Cart", "EmptyCartError"]),
  methods: {
    ConfirmCart() {
      if (this.Cart.length) {
        this.$store.commit("AddCartToDashboard");

        this.$router.push("/");
      } else {
        this.$store.state.EmptyCartError = true;
      }
    },
  },
};
